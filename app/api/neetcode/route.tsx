...
class NeetCode_ {
  ...
  static async update() {
    ...
    if (Date.now() - NeetCode_.lastUpdate < NeetCode_.msBetween) {
      console.log(
        "using cached values since last update was ",
        new Date(this.lastUpdate).toLocaleTimeString("en-US", {
          timeZone: "PST",
        })
      );
    ...
    lc.limiter = new RateLimiter({ limit: 100, concurrent: 50, interval: 10 });
    ...
    for (const submissions of returned) {
      ...
      submissions.forEach(
        (submission) =>
          (solved = solved || submission.statusDisplay == "Accepted")
      );
...
export async function GET(_req: NextRequest) {
  ...
  Array.from(NeetCode_.problemMap.keys()).forEach((topic) => {
    ...
    NeetCode_.problemMap
      .get(topic)
      ?.forEach((problem) => problems_.push(problem));
    problems.push(problems_);
  });

  // assemble the serializable ret
  const nc = {
    solvedProblems: nc_submissions,
    topics: NeetCode_.topics,
    problems: problems,
  };
